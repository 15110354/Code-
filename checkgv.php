...
<?php
		...
		{
			$mgv=$_POST['msgv'];
			$username=$_POST['tendangky'];
			$displayname=$_POST['tengiaovien'];
			$pass=$_POST['passgv'];
			
			include('dbconnect.php');
			$sql="SELECT * FROM user WHERE UserID = $mgv and RoleID = 2";
			$query = mysqli_query($conn, $sql);
			$result = mysqli_fetch_array($query);
			$dem=mysqli_num_rows($query);
			if($dem == 1)
			{
				$sqlupdate = "UPDATE user SET UserName='$username' , Pass= '$pass' ,DisplayName='$displayname' WHERE UserID = $mgv and RoleID = 2 ";
